...
Optional: in the cloud environment's variables, set `CAMINO_NTFY_URL` to an
[ntfy](https://ntfy.sh) topic URL (e.g. `https://ntfy.sh/your-private-topic`) to get a
push alert on your phone when new high-relevance finds appear. Leave it unset for no alerts.
...
In brief: from the repo root, install requirements and run
scripts/fetch_sources.py; then for every item in state/new_items.json, translate it
to English if needed, write a short title and 1-3 sentence summary, and score its
relevance 0-100 per the skill's rubric (applying each item's weight). Append the
results to state/findings.json, de-duplicating by id and marking this run's items
...
If CAMINO_NTFY_URL is set and any new finding scores >= 35, send the ntfy digest.
